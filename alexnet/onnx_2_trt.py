    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(
        *EXPLICIT_BATCH
        ...
        profile.set_shape(
            ...
            (MAX_BATCH_SIZE, net_c, net_h, net_w),
        )  # max shape
...
def main():
    ...
    parser.add_argument(
        "-c", "--category_num", type=int, help="number of object categories (obsolete)"
    )
    parser.add_argument(
        "-m",
        "--model",
    ...
    parser.add_argument(
        "--int8", action="store_true", help="build INT8 TensorRT engine"
    )
    parser.add_argument(
        "--dla_core",
        type=int,
    ...
    engine = build_engine(args.model, args.int8, args.dla_core, args.verbose)
    if engine is None:
