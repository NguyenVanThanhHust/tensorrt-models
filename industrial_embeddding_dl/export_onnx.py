...
class SegModel(nn.Module):
    def __init__(self):
        ...
        self.net = models.segmentation.fcn_resnet50(
            pretrained=True, aux_loss=False
        ).cuda()
    ...
    def forward(self, x):
        ...
        x = nn.functional.interpolate(
            x, isize, mode="bilinear"
        )  # uncomment if you want onnx to include interpolation
        return x
    ...
    def overlay(self, bgr, mask):
        ...
        colored = cv2.resize(
            colored, (bgr.shape[1], bgr.shape[0]), interpolation=cv2.INTER_CUBIC
        )
...
if __name__ == "__main__":
    model = SegModel()
    model.export_onnx("./weights/segmodel.onnx")
