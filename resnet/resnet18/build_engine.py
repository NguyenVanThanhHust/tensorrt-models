...
def get_args():
    parser = argparse.ArgumentParser(prog="Resnet18 in Tensor RT")
    parser.add_argument(
        "--input_path", type=str, default="resnet18.wts", help="input wts model file"
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="resnet18.engine",
...
def create_resnet_engine(
    ...
    relu4 = basicBlock(network, weight_map, relu3.get_output(0), 64, 128, 2, "layer2.0.")
    relu5 = basicBlock(network, weight_map, relu4.get_output(0), 128, 128, 1, "layer2.1.")

    relu6 = basicBlock(network, weight_map, relu5.get_output(0), 128, 256, 2, "layer3.0.")
    relu7 = basicBlock(network, weight_map, relu6.get_output(0), 256, 256, 1, "layer3.1.")

    relu8 = basicBlock(network, weight_map, relu7.get_output(0), 256, 512, 2, "layer4.0.")
    relu9 = basicBlock(network, weight_map, relu8.get_output(0), 512, 512, 1, "layer4.1.")

    pool2 = network.add_pooling_nd(
        relu9.get_output(0), window_size=trt.DimsHW(7, 7), type=trt.PoolingType.AVERAGE
    )
    assert pool2
    pool2.stride_nd = (1, 1)
